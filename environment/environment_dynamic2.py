...
class GridEnv(AECEnv):
    ...
    def __init__(self, grid_size=28):
        ...
        self.image_switch_interval = 300  # how often to advance the image

        # visualisation parameters
        self.visualisation_type = 'pygame'
        self.render_wait_millis = 1
...
': np.zeros(self.images[0].shape),
    ...
    def render(self, mode='human'):
        ...
        # if not correct, is at least the old classification?
        ...
        self.renderMatrix(((self.infos[self.agent_selection]['confidence'] * 2) - 1.0), 560, 0, scale)
...
if __name__ == '__main__':
    ...
    ix1 = np.random.randint(0, x_test.shape[0], num_iterations)
    ix2 = np.random.randint(0, x_test.shape[0], num_iterations)
    x_test1 = x_test[ix1]
    x_test2 = x_test[ix2]
    labels1 = y_test[ix1]
    labels2 = y_test[ix2]
    ...
    for i in range(num_iterations):
        ...
        # print("Label 1 is " + str(labels1[i]))
        # print("Label 2 is " + str(labels2[i]))

        env.reset(image1=x_test1[i], image2=x_test2[i], label1=labels1[i], label2=labels2[i])  # load the image
