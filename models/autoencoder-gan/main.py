...
class Autoencoder():
    def __init__(self):
        ...
        self.full_model.compile(loss='binary_crossentropy', optimizer=Adam(0.0008, 0.5))
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 5 == 0:
                gen_imgs = self.decoder.predict(self.encoder.predict([masked_imgs, masks], verbose=0), verbose=0)
                d_loss_real, d_acc_real = self.discriminator.train_on_batch(imgs, valid)
                d_loss_fake, d_acc_fake = self.discriminator.train_on_batch(gen_imgs, fake)
                d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)

                # train the full combined model (autoencoder with discriminator loss)
                loss2 = self.full_model.train_on_batch([masked_imgs, masks], valid)



            # train the autoencoder model (encoder and decoder, mse loss with original images)
            ...
            # print the progress
            print(f"{epoch} [DLoss: {d_loss_real}, {d_loss_fake} AE Loss: {loss} Gen Loss: {loss2}]")
...
if __name__ == '__main__':
    ae = Autoencoder()
    ae.train(epochs=8000, batch_size=32, sample_interval=400)
