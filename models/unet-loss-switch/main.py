...
class U_Net():
    def __init__(self):
        ...
        self.discriminator.compile(loss=['binary_crossentropy'], optimizer=Adam(0.0003, 0.5), metrics=['accuracy'])
        ...
        # load the mnist dataset from file instead
    ...
    def train(self, epochs, a, b, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            prob_adv = np.clip(((epoch - a) / (b - a)), 0, 1)
            isAdv = np.random.choice([True, False], p=[prob_adv, 1 - prob_adv])
            ...
            if not isAdv:
                ...
                g_loss = self.generator.train_on_batch([masked_imgs, masks], imgs)
                print(f"{epoch} [Generator MSE: {g_loss}]")
            ...
            # if epoch == 0:
            #     self.f.write("epoch,d_loss,d_acc,g_loss,g_acc\n")
            # self.f.write(f"{epoch},{d_loss[0]},{d_loss[1]},{g_loss_disc[0]},{g_loss_disc[1]}\n")
    ...
    def backup_model(self):
        self.discriminator.save('saved_model/v2_dis.keras')
        self.generator.save('saved_model/v2_gen.keras')
        self.classifier.save('saved_model/v2_clas.keras')


if __name__ == '__main__':
    model = U_Net()
    model.train(epochs=2001, a=500, b=1500, batch_size=64, sample_interval=100)
