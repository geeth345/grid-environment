...
from keras.layers import GaussianNoise, Dropout, Resizing
...
class ACGAN():
    ...
    def __init__(self):
        ...
        self.discriminator.compile(loss=['binary_crossentropy', 'sparse_categorical_crossentropy'],
                                   optimizer=Adam(0.00015, 0.5, 0.9), metrics=['accuracy'], loss_weights=[0.42, 0.58])
        ...
        self.combined.compile(loss=['binary_crossentropy', 'sparse_categorical_crossentropy'],
                              optimizer=Adam(0.00020, 0.5, 0.9), metrics=['accuracy'], loss_weights=[0.42, 0.58])
        ...
        data = np.load(self.data_source, allow_pickle=True)
    ...
    def train(self, epochs, batch_size, sample_interval):
        ...
        self.metrics_file.write(
            "epoch,d_real_auth_acc,d_real_class_acc,d_fake_auth_acc,d_fake_class_acc,g_auth_acc,g_class_acc\n")
        ...
        for epoch in range(epochs):
            ...
            # print the progress
            print(
                f"{epoch} [Auth Loss: {auth_loss}, Auth Acc: {auth_acc}, Class Loss: {class_loss}, Class Acc: {class_acc}]")
...
if __name__ == '__main__':
    net = ACGAN()
    net.train(epochs=5001, batch_size=128, sample_interval=500)
