...
class UNet():
    ...
    def __init__(self, checkpoint=None):
        ...
        self.data_source = '../../data/masked_dynamic200_600_0.7_exponential_0.005.npz'
        ...
        if not checkpoint:
            ...
            self.generator = load_model(f'saved_model/gen_{checkpoint}.keras', compile=False)
            self.discriminator = load_model(f'saved_model/disc_{checkpoint}.keras', compile=False)
            self.metrics_file = open('metrics.csv', 'a')

        self.generator.compile()
        self.discriminator.compile(loss=['binary_crossentropy', 'sparse_categorical_crossentropy'], optimizer=Adam(0.00004, 0.5, 0.9), metrics=['accuracy'], loss_weights=[0.4, 0.6])
        ...
        self.combined.compile(loss=['binary_crossentropy', 'sparse_categorical_crossentropy'], optimizer=Adam(0.00012, 0.5, 0.9), metrics=['accuracy'], loss_weights=[0.4, 0.6])
        ...
        self.X_train = data['X_train2']
        self.y_train = data['y_train2']
        self.X_test = data['X_test2']
        self.y_test = data['y_test2']
        self.X_train_masked = data['X_train_masked']
        self.X_masks = data['X_train_masks']
        ...
        self.cnn = load_model('../../models_final/_classifier/mnist_cnn.h5')
    ...
    def train(self, epochs, batch_size, sample_interval):
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            if (epoch - self.start_epoch) % sample_interval == 0:
...
if __name__ == '__main__':
    unet = UNet()
    unet.train(epochs=10001, batch_size=64, sample_interval=200)
