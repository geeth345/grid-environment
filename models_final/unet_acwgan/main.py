...
class UNet():
    ...
    def __init__(self):
        ...
        d_optimiser = Adam(0.0002, 0.6, 0.9)
        g_optimiser = Adam(0.0002, 0.6, 0.9)
    ...
    class WGAN(Model):
        ...
        def train_step(self, batch_size, trainGenerator=True):
            ...
            for i in range(5):
                ...
                labels = self.parent.y_train[idx]
                ...
                with tf.GradientTape() as tape:
                    ...
                    fake_logits, classes1 = self.discriminator(fake_images, training=True)
                    # discriminator output on the real images
                    real_logits, classes2 = self.discriminator(real_images, training=True)

                    classes_loss1 = sparse_categorical_crossentropy(labels, classes1)
                    classes_loss2 = sparse_categorical_crossentropy(labels, classes2)
                    classes_loss = np.mean((classes_loss1 + classes_loss2) / 2)

                    # calculator the discriminator loss
                    d_cost = self.d_loss_fn(real_img=real_logits, fake_img=fake_logits)
                    # calculate gradient penalty
                    gp = self.gradient_penalty(batch_size, real_images, fake_images)
                    # add the gradient penalty to the discriminator loss, and also add the classifier loss
                    d_loss = (d_cost + gp * 10) * (classes_loss * 0.5)
            ...
            if trainGenerator:
                ...
                with tf.GradientTape() as tape:
                    ...
                    # get discriminator output for fake images
                    gen_img_logits, gen_image_classes = self.discriminator(generated_images, training=True)
                    classes_loss = np.mean(sparse_categorical_crossentropy(labels, gen_image_classes))
                    # calculate the generator loss
                    g_loss = self.g_loss_fn(gen_img_logits) * (classes_loss * 0.5)
    ...
    def build_discriminator(self):
        ...
        x = Conv2D(16, kernel_size=4, strides=2, input_shape=self.input_shape, padding="same")(image)
    ...
    def backup_model(self, epoch):
        self.generator.save(f'saved_model/gen2.keras')
        self.discriminator.save(f'saved_model/disc2.keras')
